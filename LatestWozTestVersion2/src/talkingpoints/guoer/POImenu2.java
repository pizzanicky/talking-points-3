...
import talkingpoints.guoer.BTlist.MyCount;
...
public class POImenu2 extends GestureUI {
...
//	
    ...
    private static boolean flagForUpDown=false; 
...
//		this.options.add("What's around" + pageName);
		...
		Log.d(getClass().getSimpleName(), "TPID"+ID);
...
//			p = new MsgParser(GET_POI_INFO + MAC + ".xml");
		...
		if(p.getDescription().length()>0)
			pageName = "Description. "+p.getDescription()+"Swipe down to hear more information about."+getIntent().getStringExtra("POIname");
		else 
			pageName = "Swipe down to hear information about."+getIntent().getStringExtra("POIname");

		super.onCreate(savedInstanceState);
	//	Toast.makeText(this,"TPDI ???"+p.getName(),Toast.LENGTH_SHORT).show();

		Log.d(getClass().getSimpleName(), "gesture(selected)"+GestureUI.selected);
		Log.d(getClass().getSimpleName(), "optionssize"+options.size());
		...
		for (int i = 0; i < p.getSetionNameValue().size();) {
			...
			if (!p.getSectionTextValue().get(i).equalsIgnoreCase("")) {
 				options.add(p.getSetionNameValue().get(i)+"."+p.getSectionTextValue().get(i));
//				options.add(p.getSetionNameValue().get(i));
		...
		if (p.getCommentId().size() >= 1)
		{
			...
			for (int j = 0; j < p.getCommentId().size(); j++) {
				options.add(p.getCommentTitle().get(j)+"."+p.getCommentText().get(j));
			}
		}
		...
		{
			if(p.getDescription().length()>0)
				options.add("Description."+p.getDescription());
		}
...
////						intent = new Intent(POImenu2.this, Content.class);
////						intent.putExtra("content", content);
////						startActivity(intent);
////						break;
//
...
//					playSound(NEXT_PAGE);
//					
//					Intent intent = new Intent(POImenu2.this, Comments.class);
...
//				{
//
//					MyCount counter = new MyCount(4000, 1000);
//	                counter.start();
//					 
//
//				}
    		...
    		if(FirstX>0||FirstY>0)
    		{
    			...
    			try{
    				...
    				else if(LastX - FirstX >SWIPE_MIN_DISTANCE_RIGHT_LEFT&& yD< CHECK_DISTANCE) 
    				{
    					...
    					if(flagForUpDown==false||options.size()==0)
    						this.sayPageName();
    					else
	    					this.sayPageName(this.options.get(GestureUI.selected));
	...
	public boolean onKeyDown(int keyCode_1, KeyEvent event) {
		...
		if (keyCode_1 == KeyEvent.KEYCODE_DPAD_CENTER) {
			AngleCalculator oc = new AngleCalculator(byCoordinateParser.getLatitude(), byCoordinateParser
 			...
 			Intent intent0 = new Intent(POImenu2.this,POIsAhead.class);
// 			intent0.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			intent0.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);

			POImenu2.this.startActivity(intent0);
//			POImenu.this.finish();
