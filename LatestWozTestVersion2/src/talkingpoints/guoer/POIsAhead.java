...
import talkingpoints.guoer.POImenu.MyCount;
		...
		super.gestureScanner.setOnDoubleTapListener(new OnDoubleTapListener() {
			public boolean onDoubleTap(MotionEvent e) {
				...
				if(options.size()!=0){
					...
					Intent intent = new Intent(POIsAhead.this, POImenu2.class);
				...
				if ((Math.abs(cur_angle - AngleCalculator.NearbyAngle.get(i)) < range)
					...
					//ground floor 
			 		NumberFormat formatter = new DecimalFormat("#0");
						...
						if((byCoordinateParser.floor.get(i)==floorForWozTest)&&((byCoordinateParser.distance.get(i)*5280)<60.00))
						{
